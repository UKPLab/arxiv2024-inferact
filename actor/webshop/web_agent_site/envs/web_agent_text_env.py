...
class WebAgentTextEnv(gym.Env):
    ...
    def step(self, action):
        ...
        action_name, raw_action_arg = parse_action(action)
        action_arg = None
        if raw_action_arg is not None:
            action_arg = raw_action_arg.lower()
        if (action_name == 'search' and 
            ...
            ob = f"the {action} is not valid. Please check it again!"

        # Update observation, state with the new action
        if ob is None:
            ob = self.observation

        next_avaliable_actions = self.get_available_actions(with_obs=True)
        if raw_action_arg in next_avaliable_actions["clickables"]:
            next_avaliable_actions["clickables"].remove(raw_action_arg)

        ob += "\n\nAvailable Actions:\n" + str(next_avaliable_actions)
        text_list = [ob]
    ...
    def get_available_actions(self, with_obs=False):
